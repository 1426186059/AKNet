...
{
    /// <summary>
    /// 浏览器 WASM（.NET WebAssembly / Blazor browser）版本 NetClientMain。
    /// 结构与 Unity WebGL 模板（ClientTemplate/KNet.WebSocket.Client.Unity/Runtime）一致：
    /// 本文件只保留帧驱动状态机；ClientWebSocket 收发路径在浏览器不可用，
    /// 由 NetClientMain_Browser.cs 经 [JSImport] 驱动浏览器原生 WebSocket 替代。
    ...
    public partial class NetClientMain : NetClientMainBase, ClientPeerBase
    {
        ...
        // 浏览器路径没有 ClientWebSocket / 后台收发 Task，锁仅用于保护环形缓冲。
        private readonly object mWsLock = new object();

        protected string ServerIp = "";
        protected int nServerPort = 0;
        ...
        public void Update(double elapsed)
        {
            ...
            // 浏览器 WebSocket 轮询：连接事件 + 接收消息（对应 Unity WebGL 的 WebGLServicePoll）。
            BrowserServicePoll(elapsed);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected void ResetSendHeartBeatTime() { fSendHeartBeatTime = 0f; }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ReceiveHeartBeat() { fReceiveHeartBeatTime = 0f; }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected void SetSocketState(SOCKET_PEER_STATE mSocketPeerState) { this.mSocketPeerState = mSocketPeerState; }
