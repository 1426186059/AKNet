...
namespace KNet.WebSocket
{
    internal static class JslibDeployer
    {
        ...
        /// <summary>
        /// 尝试将 KNet.WebSocket.jslib 部署到 Unity 项目的 Assets/Plugins/WebGL/ 目录。
        ...
        internal static void TryDeploy()
        {
            ...
            try
            {
                ...
                string targetPath = Path.Combine(targetDir, "KNet.WebSocket.jslib");
                ...
                bDeployed = true;
                Console.WriteLine("[KNet.WebSocket] JSLib deployed to " + targetPath);
            }
            catch
            {
                // 静默失败，不影响正常逻辑
