...
namespace KNet.Tcp4.Server
{
    internal partial class NetServerMain
    {
        ...
        public bool MultiThreadingHandleConnectedSocket(Socket socket)
        {
            ...
            else
            {
                ...
                {
                    mConnectSocketQueue.Enqueue(socket);
                }
                return true;
            }
        }
        ...
        private bool CreateClientPeer()
        {
            ...
            lock (mConnectSocketQueue)
            {
                mConnectSocketQueue.TryDequeue(out socket);
            }
            if (socket != null)
            {
                ClientPeerWrap clientPeer = new ClientPeerWrap(this);
                clientPeer.HandleConnectedSocket(socket);
